 ...
 * <p>
 * The Dexter Industries Thermal Infrared Sensor reads surface temperatures of
 * objects. It is a non-contact thermometer based on the Melexis MLX90614xCC.
 * Detecting the infrared radiation from an object, the sensor can read object
 * temperatures between -90\u00b0F and 700\u00b0F (-70\u00b0C and +380\u00b0C).
 * The sensor has a high accuracy of 0.5\u00b0C and a resolution of 0.02\u00b0C.
 * <p>
 * The Thermal Infrared Sensor reads both the ambient temperature (the
 * temperature of the air around the sensor) and the surface temperature of the
 * object that the sensor is pointed towards.
 * 
 * from http://code.metager.de/source/xref/lejos/classes/src_shared/lejos/nxt/addon/DThermalIR.java
 * original source: leJOS NXT API
 * 
...
public class DThermalIR extends I2CSensor {
	...
	public float readObject() {
		...
		// retval = retval * 1.8f + 32; //TODO remove fahrenheit conversion
		// after testing
		return retval;
	}
	 ...
	 * after it was intially set after power-up with
	 * <code>setEmissivity()</code>. It doesn't seem to retrieve it from EEPROM.
	 * 
	...
	/**
	 * Set the sensor's emissivity value. Valid values are 0.01-1.0. The
	 * emissivity is stored in non-volatile memory of the sensor and will be
	 * retained even after power-off.
	 * <p>
	 * < 0.01 returns with no action. > 1.0 sets to 1.0
	 * 
	 * @param emissivity
	 *            The value to set emissivity coefficient to
	 */
	public synchronized void setEmissivity(float emissivity) {
		int intEmissivity = Math.round(emissivity * MAX_EMISSIVITY);
		if (intEmissivity < 100)
			return;
		if (intEmissivity > MAX_EMISSIVITY)
			intEmissivity = MAX_EMISSIVITY;
		int retval;

		EndianTools.encodeShortBE(intEmissivity, buf, 0);
		// LCD.drawString("e=" + EndianTools.decodeUShortBE(buf, 0) + "  ",
		// 0,4);
